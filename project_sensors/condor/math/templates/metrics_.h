...
/**< modifies the input container so its median value (if ordered) is now located in the middle position. Also returns the median value. - DOES NOT WORK FOR LISTS BECAUSE last - first NOT AVAILABLE */
template <class Cont> typename Cont::value_type median(Cont& cont) {
    assert(cont.empty() == false);
    auto it = cont.begin() + floor(cont.size()/2);
    std::nth_element(cont.begin(), it, cont.end());
    return *it;
}

/**< modifies the input vector so its median value (if ordered) is now located in the middle position. Also returns the median value. ONLY VALID FOR VECTORS */
template<class Cont> Cont median_vector(std::vector<Cont>& cont) {
    assert(cont.empty() == false);
    typename std::vector<Cont>::iterator it = cont.begin() + floor(cont.size()/2);
    std::nth_element(cont.begin(), it, cont.end());
    return *it;
}
...
/**< returns the median absolute deviation of the values in the input vector (also requires the median) */
template <class Cont> typename Cont::value_type mad(const Cont& cont, const typename Cont::value_type& median) {
    Cont diff(cont.size());
    auto itc = cont.begin();
    for (auto it = diff.begin(); it != diff.end(); ++it, ++itc) {
        (*it) = std::fabs((*itc) - median);
    }
    return math::median(diff);
}

/**< returns the median absolute deviation of the values in the input vector (also requires the median) - ONLY VALID FOR VECTORS */
template <class Cont> Cont mad_vector(const std::vector<Cont>& cont, const Cont& median) {
    std::vector<Cont> diff(cont.size());
    auto itc = cont.begin();
    for (auto it = diff.begin(); it != diff.end(); ++it, ++itc) {
        (*it) = std::fabs((*itc) - median);
    }
    return math::median_vector(diff);
}
