...
double math::low_pass_single::eval_without_modify(const double& u) const {
    // note that if u == y_prev, then output = y_prev
    return _beta * _y_prev + _alpha * u;
}
...
Eigen::Vector3d math::low_pass_triple::eval_without_modify(const Eigen::Vector3d& u) const {
    // note that if u == y_prev, then output = y_prev
    return _beta * _y_prev + _alpha * u;
}
/* evaluate filter without modifying its state */
