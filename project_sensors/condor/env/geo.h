...
public:
    ...
    explicit geo(env::logic::ZONE_ID);
    /**< constructor based on location, realism enumerator, and realism seed. Seed only applies to realistic
     * (not model) gravitation and magnetism. */
    geo(env::logic::ZONE_ID zone_id, env::logic::REALISM_ID, const int& seed);
    /**< copy constructor */
    ...
    virtual Eigen::Matrix3d jacobian_cartesian_geodetic(const env::geodetic_coord& gisu, const double& N_m) const;
    ...
    * It first calls the compute_gravity_n_model and then modifies it to add stochastic realism. */
    ...
    /**< computes the absolute speed in NED based on the time derivative of the geodetic coordinates and the radii of curvature */
    ...
    static Eigen::Matrix3d jac_xgdtdot_wrt_vned(const env::geodetic_coord& x_gdt_rad_m, const double& N_m, const double& M_m);

    /**< ===== ===== ===== Earth Angular Velocity ===== ===== ===== */
    /**< computes the Earth angular velocity [rps] (ECEF with respect to IRS) viewed in ECEF */
    virtual ang::so3_tangent compute_wiee_rps() const;
    /**< computes the Earth angular velocity [rps] (ECEF with respect to IRS) viewed in NED */
	virtual ang::so3_tangent compute_wien_rps(const double& phi_rad) const;
    /**< computes the jacobian of the Earth angular velocity viewed in ECEF with respect to the ground velocity viewed in ECEF */
	virtual Eigen::Matrix3d jac_wiee_wrt_ve() const;
    /**< computes the jacobian of the Earth angular velocity viewed in NED with respect to the ground velocity viewed in NED */
    virtual Eigen::Matrix3d jac_wien_wrt_vn() const;

    /**< ===== ===== ===== Motion Angular Velocity ===== ===== ===== */
	/**< computes the motion angular velocity [rps] (NED with respect to ECEF) viewed in ECEF */
    virtual ang::so3_tangent compute_wene_rps(const Eigen::Vector3d& v_n_mps, const double& N_m, const double& M_m, const double& lambda_rad, const double& phi_rad, const double& h_m) const;
    /**< computes the motion angular velocity [rps] (NED with respect to ECEF) viewed in ECEF */
    virtual ang::so3_tangent compute_wene_rps(const double& lambdadot_rps, const double& phidot_rps, const double& lambda_rad) const;
    /**< computes the motion angular velocity [rps] (NED with respect to ECEF) viewed in ECEF */
    virtual ang::so3_tangent compute_wene_rps(const double& N_m, const double& M_m, const double& lambda_rad, const double& phi_rad, const double& h_m, const Eigen::Vector3d& v_e_mps) const;
    /**< computes the motion angular velocity [rps] (NED with respect to ECEF) viewed in NED */
	virtual ang::so3_tangent compute_wenn_rps(const Eigen::Vector3d& v_n_mps, const double& N_m, const double& M_m, const double& phi_rad, const double& h_m) const;
    /**< computes the motion angular velocity [rps] (NED with respect to ECEF) viewed in NED */
    virtual ang::so3_tangent compute_wenn_rps(const double& lambdadot_rps, const double& phidot_rps, const double& phi_rad) const;
    /**< computes the jacobian of the motion angular velocity viewed in ECEF with respect to the ground velocity viewed in ECEF */
    virtual Eigen::Matrix3d jac_wene_wrt_ve(const double& N_m, const double& M_m, const double& lambda_rad, const double& phi_rad, const double& h_m) const;
    /**< computes the jacobian of the motion angular velocity viewed in NED with respect to the ground velocity viewed in NED */
    virtual Eigen::Matrix3d jac_wenn_wrt_vn(const double& N_m, const double& M_m, const double& phi_rad, const double& h_m) const;

    /**< ===== ===== ===== Coriolis Acceleration ===== ===== ===== */
    /**< computes the Coriolis acceleration [mps2] viewed in ECEF */
    virtual Eigen::Vector3d compute_coriolis_e(const Eigen::Vector3d& v_e_mps, const ang::so3_tangent& w_iee_rps) const;
    /**< computes the Coriolis acceleration [mps2] viewed in ECEF */
    virtual Eigen::Vector3d compute_coriolis_e(const Eigen::Vector3d& v_e_mps) const;
    /**< computes the Coriolis acceleration [mps2] viewed in NED */
    virtual Eigen::Vector3d compute_coriolis_n(const Eigen::Vector3d& v_n_mps, const ang::so3_tangent& w_ien_rps) const;
    /**< computes the Coriolis acceleration [mps2] viewed in NED */
    ...
    virtual Eigen::Matrix3d jac_coriolis_e_wrt_ve(const ang::so3_tangent& w_iee_rps) const;
    /**< computes the jacobian of the Coriolis acceleration viewed in eCEF with respect to the ground speed viewed in ECEF */
    virtual Eigen::Matrix3d jac_coriolis_e_wrt_ve() const;
    /**< computes the jacobian of the Coriolis acceleration viewed in NED with respect to the ground speed viewed in NED */
    virtual Eigen::Matrix3d jac_coriolis_n_wrt_vn(const ang::so3_tangent& w_ien_rps) const;
    /**< computes the jacobian of the Coriolis acceleration viewed in NED with respect to the ground speed viewed in NED */
    virtual Eigen::Matrix3d jac_coriolis_n_wrt_vn(const double& phi_rad) const;

    /**< ===== ===== ===== Transport Acceleration ===== ===== ===== */
    /**< computes the transport acceleration [mps2] viewed in ECEF */
    virtual Eigen::Vector3d compute_transport_e(const double& N_m, const double& h_m, const double& lambda_rad, const double& phi_rad) const;
...
public:
    ...
    geo_mix() = delete;
    /**< constructor based on location. Model and real gravitation and magnetic fields coincide. */
    explicit geo_mix(env::logic::ZONE_ID);
    /**< constructor based on location, realism enumerator, and realism seed. Seed only applies to realistic
     * (not model) gravitation and magnetism. */
    geo_mix(env::logic::ZONE_ID zone_id, env::logic::REALISM_ID, const int& seed);
    /**< copy constructor */
    geo_mix(const geo_mix&) = delete;
    /**< move constructor */
    geo_mix(geo_mix&&) = delete;
    /**< destructor */
    virtual ~geo_mix() override = default;
    /**< copy assignment */
...
public:
    ...
    geo_mix11() = delete;
    /**< constructor based on location. Model and real gravitation and magnetic fields coincide. */
    explicit geo_mix11(env::logic::ZONE_ID zone_id) : geo_mix(zone_id) {}
    /**< constructor based on location, realism enumerator, and realism seed. Seed only applies to realistic
     * (not model) gravitation and magnetism. */
    geo_mix11(env::logic::ZONE_ID zone_id, env::logic::REALISM_ID realism_id, const int& seed) : geo_mix(zone_id, realism_id, seed) {}
    /**< copy constructor */
    ...
    /**< ===== ===== ===== Earth Angular Velocity ===== ===== ===== */
    /**< computes the Earth angular velocity [rps] (ECEF with respect to IRS) viewed in ECEF */
    ang::so3_tangent compute_wiee_rps() const override {return ang::so3_tangent(Eigen::Vector3d::Zero());}
    /**< computes the Earth angular velocity [rps] (ECEF with respect to IRS) viewed in NED */
    ang::so3_tangent compute_wien_rps(const double& phi_rad) const override {return ang::so3_tangent(Eigen::Vector3d::Zero());}
    /**< computes the jacobian of the Earth angular velocity viewed in ECEF with respect to the ground velocity viewed in ECEF */
    Eigen::Matrix3d jac_wiee_wrt_ve() const override {return Eigen::Matrix3d::Zero();}
    /**< computes the jacobian of the Earth angular velocity viewed in NED with respect to the ground velocity viewed in NED */
    Eigen::Matrix3d jac_wien_wrt_vn() const override {return Eigen::Matrix3d::Zero();}

    /**< ===== ===== ===== Motion Angular Velocity ===== ===== ===== */
    /**< computes the motion angular velocity [rps] (NED with respect to ECEF) viewed in ECEF. */
    ang::so3_tangent compute_wene_rps(const Eigen::Vector3d& v_n_mps, const double& N_m, const double& M_m, const double& lambda_rad, const double& phi_rad, const double& h_m) const override {return ang::so3_tangent(Eigen::Vector3d::Zero());}
    /**< computes the motion angular velocity [rps] (NED with respect to ECEF) viewed in ECEF. */
    ang::so3_tangent compute_wene_rps(const double& lambdadot_rps, const double& phidot_rps, const double& lambda_rad) const override {return ang::so3_tangent(Eigen::Vector3d::Zero());}
    /**< computes the motion angular velocity [rps] (NED with respect to ECEF) viewed in ECEF */
    ang::so3_tangent compute_wene_rps(const double& N_m, const double& M_m, const double& lambda_rad, const double& phi_rad, const double& h_m, const Eigen::Vector3d& v_e_mps) const override {return ang::so3_tangent(Eigen::Vector3d::Zero());}
    /**< computes the motion angular velocity [rps] (NED with respect to ECEF) viewed in NED. */
    ang::so3_tangent compute_wenn_rps(const Eigen::Vector3d& v_n_mps, const double& N_m, const double& M_m, const double& phi_rad, const double& h_m) const override {return ang::so3_tangent(Eigen::Vector3d::Zero());}
    /**< computes the motion angular velocity [rps] (NED with respect to ECEF) viewed in NED. */
    ang::so3_tangent compute_wenn_rps(const double& lambdadot_rps, const double& phidot_rps, const double& phi_rad) const override {return ang::so3_tangent(Eigen::Vector3d::Zero());}
    /**< computes the jacobian of the motion angular velocity viewed in ECEF with respect to the ground velocity viewed in ECEF */
    Eigen::Matrix3d jac_wene_wrt_ve(const double& N_m, const double& M_m, const double& lambda_rad, const double& phi_rad, const double& h_m) const override {return Eigen::Matrix3d::Zero();}
    /**< computes the derivative of the angular speed of ECEF to NED viewed in NED with respect to ground speed */
    Eigen::Matrix3d jac_wenn_wrt_vn(const double& N_m, const double& M_m, const double& phi_rad, const double& h_m) const override {return Eigen::Matrix3d::Zero();}

    /**< ===== ===== ===== Coriolis Acceleration ===== ===== ===== */
    /**< computes the Coriolis acceleration [mps2] viewed in ECEF */
    Eigen::Vector3d compute_coriolis_e(const Eigen::Vector3d& v_e_mps, const ang::so3_tangent& w_iee_rps) const override {return Eigen::Vector3d::Zero();}
    /**< computes the Coriolis acceleration [mps2] viewed in ECEF */
    Eigen::Vector3d compute_coriolis_e(const Eigen::Vector3d& v_e_mps) const override {return Eigen::Vector3d::Zero();}
    /**< computes the Coriolis acceleration [mps2] viewed in NED */
    Eigen::Vector3d compute_coriolis_n(const Eigen::Vector3d& v_n_mps, const ang::so3_tangent& w_ien_rps) const override {return Eigen::Vector3d::Zero();}
    /**< computes the Coriolis acceleration [mps2] viewed in NED */
    Eigen::Vector3d compute_coriolis_n(const Eigen::Vector3d& v_n_mps, const double& phi_rad) const override {return Eigen::Vector3d::Zero();}
    /**< computes the jacobian of the Coriolis acceleration viewed in ECEF with respect to the ground speed viewed in ECEF */
    Eigen::Matrix3d jac_coriolis_e_wrt_ve(const ang::so3_tangent& w_iee_rps) const override {return Eigen::Matrix3d::Zero();}
    /**< computes the jacobian of the Coriolis acceleration viewed in eCEF with respect to the ground speed viewed in ECEF */
    Eigen::Matrix3d jac_coriolis_e_wrt_ve() const override {return Eigen::Matrix3d::Zero();}
    /**< computes the jacobian of the Coriolis acceleration viewed in NED with respect to the ground speed viewed in NED */
    Eigen::Matrix3d jac_coriolis_n_wrt_vn(const ang::so3_tangent& w_ien_rps) const override {return Eigen::Matrix3d::Zero();}
    /**< computes the jacobian of the Coriolis acceleration viewed in NED with respect to the ground speed viewed in NED */
    Eigen::Matrix3d jac_coriolis_n_wrt_vn(const double& phi_rad) const override {return Eigen::Matrix3d::Zero();}

    /**< ===== ===== ===== Transport Acceleration ===== ===== ===== */
    /**< computes the transport acceleration [mps2] viewed in ECEF */
    Eigen::Vector3d compute_transport_e(const double& N_m, const double& h_m, const double& lambda_rad, const double& phi_rad) const override {return Eigen::Vector3d::Zero();}
    /**< computes the transport acceleration [mps2] viewed in NED */
    Eigen::Vector3d compute_transport_n(const double& N_m, const double& h_m, const double& phi_rad) const override {return Eigen::Vector3d::Zero();}
    /**< computes the jacobian of the transport acceleration viewed in ECEF with respect to the ground velocity viewed in ECEF */
    Eigen::Matrix3d jac_transport_e_wrt_ve() const override {return Eigen::Matrix3d::Zero();}
    /**< computes the jacobian of the transport acceleration viewed in NED with respect to the ground velocity viewed in NED */
    Eigen::Matrix3d jac_transport_n_wrt_vn() const override {return Eigen::Matrix3d::Zero();}
