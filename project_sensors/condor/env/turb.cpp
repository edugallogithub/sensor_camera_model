...
Eigen::Vector3d env::turb_zero::compute_wind_high_frequency_twomils_bfs(const double& t_sec, const double& height_m, const double& chiwind20_rad, const ang::rodrigues& q_nb) const {
    return {0., 0., 0.};
};
...
Eigen::Vector3d env::turb_dryden::compute_wind_high_frequency_twomils_bfs(const double& t_sec, const double& height_m, const double& chiwind20_rad, const ang::rodrigues& q_nb) const {
	...
	if (height_m <= _height_low_m) { //  low altitude
		...
		return q_nb / wind_low_ned_fps * 0.3048 * _factor;
	}
		...
		ang::euler euler_twsned(-chiwind20_rad, 0.0, 0.0);
        Eigen::Vector3d wind_low_ned_fps = euler_twsned / wind_low_tws_fps;
        Eigen::Vector3d wind_low_bfs_mps = q_nb / wind_low_ned_fps * 0.3048;
