...
public:
    ...
    virtual Eigen::Vector3d compute_wind_high_frequency_twomils_bfs(const double& t_sec, const double& height_m, const double& chiwind20_rad, const ang::rodrigues& q_nb) const = 0;
...
public:
    ...
    /**< computes the high frequency wind in BFS [mps] based on the input time, the height above the terrain, the wind direction at an altitude of 20 [ft],
     * and the rotation matrix from NED to BFS. CAUTION: It is only valid if the input time is a multiple of 0.002 [sec]. */
    Eigen::Vector3d compute_wind_high_frequency_twomils_bfs(const double& t_sec, const double& height_m, const double& chiwind20_rad, const ang::rodrigues& q_nb) const override;
