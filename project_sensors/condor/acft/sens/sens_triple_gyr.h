...
public:
    ...
    ang::so3_tangent eval_noB_noM_noR(const ang::so3_tangent& w_ibb_rps, Eigen::Vector3d& Obias, Eigen::Vector3d& Oscalecross, Eigen::Vector3d& Oerror);
    /**< return new sensor measurement based on sensor input, also filling up the bias, scale cross error, and their sum (full error minus noise) */
    ang::so3_tangent eval_noM_noR(const ang::so3_tangent& w_ibb_rps, Eigen::Vector3d& Obias, Eigen::Vector3d& Oscalecross, Eigen::Vector3d& Oerror);
    /**< return new sensor measurement based on sensor input, also filling up the bias, scale cross error, and their sum (full error minus noise) */
    ang::so3_tangent eval_noR(const ang::so3_tangent& w_ibb_rps, Eigen::Vector3d& Obias, Eigen::Vector3d& Oscalecross, Eigen::Vector3d& Oerror);
    /**< return new sensor measurement based on sensor input, also filling up the bias, scale cross error, and their sum (full error minus noise) */
    ang::so3_tangent eval_complete(const ang::so3_tangent& w_ibb_rps, Eigen::Vector3d& Obias, Eigen::Vector3d& Oscalecross, Eigen::Vector3d& Oerror);
