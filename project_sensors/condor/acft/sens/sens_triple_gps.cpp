...
_limit(100.0), _safety(20.0), _count(0), _flag_used(false), _first_pos_error_ned(Eigen::Vector3d::Zero()) {
...
const Eigen::Vector3d& sens::sens_triple_gps::get_first_pos_error_ned() const {
    if (_flag_used == true) {
        return _first_pos_error_ned;
    }
    else {
        throw std::runtime_error("Method eval_pos_error needs to be called first.");
    }
}
/* returns NED position error resulting from first use of this class */
