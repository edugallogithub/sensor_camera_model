...
_sigma_euler_bp_deg(sigma_euler_bp_deg) {

    // seed generator
    std::ranlux24_base Ogen(acft_seed);
    ...
    _Trpb_m_est << _Trpb_m_truth(0) + _sigma_T_rpb_m * Odist(Ogen),
                   _Trpb_m_truth(1) + _sigma_T_rpb_m * Odist(Ogen),
                   _Trpb_m_truth(2) + _sigma_T_rpb_m * Odist(Ogen);

    // true Euler angles from body to platform
    _euler_bp_truth.set_yaw_rad  (_sigma_yaw_bp_deg   * Odist(Ogen) * math::constant::D2R());
    _euler_bp_truth.set_pitch_rad(_sigma_pitch_bp_deg * Odist(Ogen) * math::constant::D2R());
    _euler_bp_truth.set_bank_rad (_sigma_bank_bp_deg  * Odist(Ogen) * math::constant::D2R());

    // estimated Euler angles from body to platform
    _euler_bp_est.set_yaw_rad  (_euler_bp_truth.get_yaw_rad()   + _sigma_euler_bp_deg * Odist(Ogen) * math::constant::D2R());
    _euler_bp_est.set_pitch_rad(_euler_bp_truth.get_pitch_rad() + _sigma_euler_bp_deg * Odist(Ogen) * math::constant::D2R());
    _euler_bp_est.set_bank_rad (_euler_bp_truth.get_bank_rad()  + _sigma_euler_bp_deg * Odist(Ogen) * math::constant::D2R());
