...
ang::so3_tangent sens::sens_triple_gyr::eval_noB_noM_noR(const ang::so3_tangent& w_ibb_rps, Eigen::Vector3d& Obias, Eigen::Vector3d& Oscalecross, Eigen::Vector3d& Oerror) {
    ...
    return ang::so3_tangent(w_ibb_rps() + Oerror + _sigma_v_Delta_tn05 * Eigen::Vector3d(_dist_run(_gen_run), _dist_run(_gen_run),_dist_run(_gen_run)));
}
...
ang::so3_tangent sens::sens_triple_gyr::eval_noM_noR(const ang::so3_tangent& w_ibb_rps, Eigen::Vector3d& Obias, Eigen::Vector3d& Oscalecross, Eigen::Vector3d& Oerror) {
    this->grow_random_walk(_Sum_Nui(0));
    this->grow_random_walk(_Sum_Nui(1));
    this->grow_random_walk(_Sum_Nui(2));
...
ang::so3_tangent sens::sens_triple_gyr::eval_noR(const ang::so3_tangent& w_ibb_rps, Eigen::Vector3d& Obias, Eigen::Vector3d& Oscalecross, Eigen::Vector3d& Oerror) {
    this->grow_random_walk(_Sum_Nui(0));
    this->grow_random_walk(_Sum_Nui(1));
    this->grow_random_walk(_Sum_Nui(2));
    Obias = _B0Nu0 + _sigma_u_Delta_t05 * _Sum_Nui;
...
ang::so3_tangent sens::sens_triple_gyr::eval_complete(const ang::so3_tangent& w_ibb_rps, Eigen::Vector3d& Obias, Eigen::Vector3d& Oscalecross, Eigen::Vector3d& Oerror) {
...
sens::sens_triple_gyr* sens::sens_triple_gyr::create_gyroscope(sens::logic::GYR_ID gyr_id, sens::logic::BAND_ID band_id, const int& acft_seed, const int& run_seed, const sens::platform& Oplat, const double& Deltat_sec) {
    ...
    switch(gyr_id) {
        ...
        case sens::logic::gyr_id_noise_only:
            ...
            break;
        case sens::logic::gyr_id_base_old:
            Pres = new sens::sens_triple_gyr(1.42e-4 * d2r, 4.30e-3 * d2r, 3.00e-4, 8.70e-4, 2.00e-2 * d2r, limit, Deltat_sec, acft_seed, run_seed, Oplat);
            break;
