...
/* ===== ====================================================================== ===== */
Eigen::Matrix3d tools::skew3(const Eigen::Vector3d& v) {
...
Eigen::Vector3d tools::skew3_inverse(const Eigen::Matrix3d& m) {
	return {m(2,1), m(0,2), m(1,0)};
}
/* returns the vector from a skew symmetric matrix */

Eigen::Matrix3d tools::right_skew3(const Eigen::Vector3d& v) {
...
Eigen::Vector3d tools::right_skew3_inverse(const Eigen::Matrix3d& m) {
...
/* ===== ===== ====================================== === ===== */
Eigen::Matrix4d tools::skew4(const Eigen::Vector4d& q) {
...
Eigen::Vector4d tools::skew4_inverse(const Eigen::Matrix4d& m) {
    return m.leftCols<1>();
}
/* returns the quaternion from a quaternion left product matrix (skew symmetric) */

Eigen::Matrix4d tools::right_skew4(const Eigen::Vector4d& q) {
...
Eigen::Vector4d tools::right_skew4_inverse(const Eigen::Matrix4d& m) {
    return m.leftCols<1>();    
}
...
/* ================================================================== */
Eigen::Matrix<double,4,3> tools::skew43(const Eigen::Vector4d& q) {
...
Eigen::Matrix4d tools::right_skew43(const Eigen::Vector3d& v) {
...
/* ===== ===== ===== Other Methods ===== ===== ===== */
/* ===== ===== ===== ============= ===== ===== ===== */
void tools::correct_yaw_rad(double& yaw_rad) {
...
void tools::correct_longitude_rad(double& lambda_rad) {
...
void tools::correct_yaw_deg(double& yaw_deg) {
    if (yaw_deg > 180.0) {
...
void tools::correct_longitude_deg(double& lambda_deg) {
    if (lambda_deg >= 360.0) {
...
double tools::angle_diff_rad(const double& a_rad, const double& b_rad) {
...
double tools::angle_diff_deg(const double& a_deg, const double& b_deg) {
...
Eigen::Matrix3d tools::itself_by_transpose(const Eigen::Vector3d& vec) {
    return vec * vec.transpose();
}
/* returns matrix resulting from multiplying the vector transpose by itself */

Eigen::Matrix3d tools::jacobian_triple(const Eigen::Vector3d& a, const Eigen::Vector3d& b) {
    //Eigen::RowVector3d atr = a.transpose();
    //Eigen::RowVector3d btr = b.transpose();
    //return Eigen::Matrix3d::Identity() * (atr * b) + a * btr - b * atr * 2.0;
    throw std::runtime_error("FUNCTION DOES NOT COMPILE BUT IT IS NOT USED. DELETEL.");
    return Eigen::Matrix3d::Zero();
}
/* given the vector (a.cross(a.cross(b))), returns its jacobian with respect to a (refer to
 * hand written note for expression). */
