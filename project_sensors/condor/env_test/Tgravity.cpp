...
void env::test::Tgravity::test_model() {
    env::geo_mix Ogeo(env::logic::zone_default); // magnetism does not matter here
...
void env::test::Tgravity::test_truth() {
    env::logic::ZONE_ID zone_id = env::logic::zone_default; // meaningless in this test as it does not affect gravity
    ...
    env::geo_mix Ogeo_model(zone_id);
    ...
    for (unsigned short seed_order = 1; seed_order <= siz; ++seed_order) {
        env::geo_mix Ogeo(zone_id, realism_id, seed_order);
